...
class GenerativeAgentMemory(BaseMemory):
    ...
    aggregate_importance: float = 0.5  # : :meta private:
    """Track the sum of the 'importance' of recent memories.

    Triggers reflection when it reaches reflection_threshold."""

    max_tokens_limit: int = 4000  # : :meta private:
    # input keys
    ...
    def _score_memory_importance(self, memory_content: str) -> float:
        ...
        prompt = PromptTemplate.from_template(
            "On the scale of 1 to 10, where 1 is purely regular street details which one can see in regular city streets"
            + " (e.g., mundane urban setting characterized by a sparse population and minimal vehicular activity. Streets are devoid of vibrant elements,"
            + " with limited greenery and visible sky. Lack of noteworthy establishments like shops, cafetarias and landmarks) and 10 is"
            + " extremely poignant (e.g., captivating urban scene which features a bustling environment with a diverse crowd and active vehicular presence."
            + " The scene composition includes a harmonious blend of greenery, buildings, and a dynamic sky. Streets are animated with life, "
            + " showcasing variety of establishments. The overall composition which creates a memorable experience.),"
